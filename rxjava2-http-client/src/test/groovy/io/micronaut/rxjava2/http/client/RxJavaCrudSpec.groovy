 ...
 * @since 1.0
 */
@Property(name = 'spec.name', value = 'RxJavaCrudSpec')
@MicronautTest
class RxJavaCrudSpec extends Specification {
    ...
    void "test it is possible to implement CRUD operations with RxJava"() {
        ...
        rxHttpClient.retrieve(HttpRequest.GET("/rxjava/books/" + book.id), Book).blockingFirst()
                .title == "The Stand"
    ...
    @Requires(property = 'spec.name', value = 'RxJavaCrudSpec')
    @Client('/rxjava/books')
